...
def fill_doctors(df):
    df['phone_number']     = df['phone_number'].fillna(0)           # numeric column → 0
...
def fill_patients(df):
    ...
    df['contact_number']     = df['contact_number'].fillna(0)       # numeric column → 0
...
def drop_remaining_nulls(datasets):
    for name in ["Appointments", "Billing"]:
        ...
        print(f"  {name:<15} dropped {before - after} rows with nulls → {after} rows remain")
...
def main():
    ...
    # Step 1: fix dtypes first (so coerced NaNs exist before filling)
    fix_dtypes(datasets)

    # Step 2: fill all NaNs
